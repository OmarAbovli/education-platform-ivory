...
const DEFAULT_TEACHER_ID = "t_demo";

export async function generateMetadata(): Promise<Metadata> {
  const data = await getTeacherProfile(DEFAULT_TEACHER_ID);
  if (!data) {
    return {
      title: "About Us | YourPlatform",
      description: "Learn more about YourPlatform and our expert teachers.",
    };
  }
  const { teacher } = data;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: new URL(`/about-us`, process.env.NEXT_PUBLIC_BASE_URL || "https://yourplatform-demo.vercel.app/"),
      images: [
        {
...
export default async function AboutUsPage() {
  ...
  const photos = await getTeacherPhotos(DEFAULT_TEACHER_ID)
    ...
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{
        __html: JSON.stringify({
          ...
          "@type": "Person",
          "name": teacher.name,
          "url": `${process.env.NEXT_PUBLIC_BASE_URL || "https://yourplatform-demo.vercel.app/"}/about-us`,
