...
import { createLiveExam, type CreateExamInput, type ExamQuestion } from "@/server/live-exam-actions"
...
export default function CreateExamPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Input format: "2025-11-23T10:21"
      // We treat this as Cairo time (UTC+2) and convert to UTC for storage
      const scheduledAtWithSeconds = scheduledAt.includes(':') && scheduledAt.split(':').length === 2 
        ? scheduledAt + ':00' 
        : scheduledAt
      const cairoDateTime = scheduledAtWithSeconds + '+02:00' // Add Cairo timezone offset
      ...
      const result = await createLiveExam(input)

      if (result.success) {
        router.push("/teacher/exams")
      } else {
        setError(result.error || "فشل إنشاء الاختبار")
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">إنشاء اختبار لايف جديد</h1>
            <p className="text-slate-400">قم بإنشاء اختبار مباشر للطلاب</p>
          </div>
        </div>
              ...
              {questions.map((question, qIndex) => (
                <div key={qIndex} className="p-4 bg-slate-800/50 rounded-lg border border-slate-700 space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-slate-500">ضع علامة ✓ على الإجابة الصحيحة</p>
                  </div>
                </div>
              ))}
            </CardContent>
